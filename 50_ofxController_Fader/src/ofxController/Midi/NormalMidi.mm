//
//  NormalMidi.cpp
//  emptyExample
//
//  Created by leico on 2014/08/31.
//
//

#include "NormalMidi.h"

namespace ofxController{

  /* ========================================== *
   * NormalMidi :: NormalMidi                   *
   * ========================================== */
  NormalMidi :: NormalMidi(void) : MidiArg(){}

  /* ========================================== *
   * NormalMidi :: NormalMidi                   *
   * ========================================== */
  NormalMidi :: NormalMidi(const NormalMidi &obj){}


  /* ========================================== *
   * NormalMidi :: NormalMidi                   *
   * ========================================== */
  NormalMidi :: NormalMidi(const int ch,           const int pitch,
                           const int min_velocity, const int max_velocity) :
    MidiArg(ch, pitch, min_velocity, max_velocity)
  {}

  /* ========================================== *
   * NormalMidi :: ~NormalMidi                  *
   * ========================================== */
  NormalMidi :: ~NormalMidi(void){}
  
  /* ========================================== *
   * NormalMidi :: touchDown                    *
   * ========================================== */
  void NormalMidi :: touchDown(float value, Core :: MidiManager *midi){
    
    midi -> sendNoteOn(
        _ch
      , _pitch
      , ofMap(value, MIN_INPUT_VALUE, MAX_INPUT_VALUE, _min_velocity, _max_velocity)
    );
  }

  /* ========================================== *
   * NormalMidi :: touchMove                    *
   * ========================================== */
  void NormalMidi :: touchMove(float value, Core :: MidiManager *midi){

    midi -> sendPolyAfterTouch(
        _ch
      , _pitch
      , ofMap(value, MIN_INPUT_VALUE, MAX_INPUT_VALUE, _min_velocity, _max_velocity)
    );
  }

  /* ========================================== *
   * NormalMidi :: touchUp                      *
   * ========================================== */
  void NormalMidi :: touchUp(float value, Core :: MidiManager *midi){

    midi -> sendNoteOff(
        _ch
      , _pitch
      , ofMap(value, MIN_INPUT_VALUE, MAX_INPUT_VALUE, _min_velocity, _max_velocity)
    );
  }

  /* ========================================== *
   * NormalMidi :: newMidiMessage               *
   * ========================================== */

  float NormalMidi :: newMidiMessage(ofxMidiMessage &msg){

    switch(msg.status){
      case MIDI_NOTE_ON :
      case MIDI_POLY_AFTERTOUCH :
        
        if( msg.channel == _ch && msg.pitch == _pitch)
          return ofMap( msg.velocity, _min_velocity, _max_velocity, MIN_INPUT_VALUE, MAX_INPUT_VALUE);
        
      break;

      case MIDI_NOTE_OFF:
        if( msg.channel == _ch && msg.pitch == _pitch)
          return ofMap(_min_velocity, _min_velocity, _max_velocity, MIN_INPUT_VALUE, MAX_INPUT_VALUE);
      break;

      default:
        return NAN;
      break;
    }

    return NAN;


  }


}
